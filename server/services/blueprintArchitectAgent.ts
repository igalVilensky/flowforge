...
const allowedRiskCategories: ReadonlySet<string> =
...
function sanitizeOptionalProposedRisks(
  parsed: unknown,
): {
  value: unknown;
  warningSummary?: string;
  ...
  if (
    !isRecord(parsed) ||
    !Array.isArray(parsed.proposed_risks)
  ) {
    return {
      value: parsed,
    };
  ...
  const proposedRisks =
    parsed.proposed_risks.filter(
      (risk) => {
        const category = isRecord(risk)
          ? risk.category
          : undefined;

        if (
          typeof category === "string" &&
          allowedRiskCategories.has(category)
        ) {
          return true;
        }

        droppedCategories.add(
          typeof category === "string" &&
            category.trim()
            ? category
            : "unknown",
  ...
  return {
    ...
    warningSummary:
      `Ignored unsupported AI risk categories: ${
        Array.from(droppedCategories).join(", ")
      }`,
  };
}
...
function safeParseJSON(
  ...
  try {
    ...
    if (!match) {
      throw new Error(
        "No valid JSON object found in Blueprint Architect Agent response.",
      );
    }
...
export function buildDeterministicBlueprintArchitectFallback(
  ...
  const needsHumanApproval =
    input.risks.requires_human_review ||
    input.signals.has_external_action;

  const hasBlocker =
    input.routerDecision.route === "reject" ||
    input.risks.categories.some(
      (category) =>
        [
          "legal",
          "medical",
  ...
  if (
    ...
    return {
      ...
      summary:
        "Deterministic content-generation fallback prepared draft assets, a review package, channel-owner approval, and a hard block on automatic publishing.",
      proposed_steps:
        blueprint.steps.map(
          (step) => ({
            id: step.id,
      ...
      proposed_human_approval_gates:
        blueprint.human_approval_gates.map(
          (gate) => ({
            id: gate.id,
            label: gate.label,
            reason: gate.reason,
            applies_to_step_ids:
              gate.applies_to_step_ids,
            required: gate.required,
          }),
        ),
      proposed_risks:
        input.risks.categories.map(
          (category) => ({
            id: `risk_${category}`,
            category,
            label:
            ...
            risk_level:
              input.risks.risk_level,
            reason:
              "Detected from the intent-only workflow analysis.",
            recommendation:
              "Keep generated assets draft-only and require explicit approval before external publishing.",
          }),
        ),
      safe_to_automate:
        blueprint.safe_to_automate,
      ...
      requires_human_approval: [
        "The channel owner must approve the post package before any external publishing.",
      ],
      ...
      open_questions:
        input.clarificationPlan.questions.map(
          (question) =>
            question.question,
        ),
      safer_alternative:
        "Prepare draft content and an internal post package, then let the channel owner review and publish manually.",
    };
  }
  ...
  return {
    ...
    reason,
    workflow_name:
      "Safe automation blueprint proposal",
    summary: hasBlocker
    ...
    proposed_steps: [
      {
        ...
        primitive: "intake",
        description:
          "Read the described trigger and input source without executing any real-world action.",
        input:
          "User-described workflow input",
        output:
          "Structured internal intake record",
        automation_policy:
          "automate",
        risk_level: "low",
      ...
      },
      {
        ...
        primitive:
          "summarization",
        description:
          "Classify the request and produce a concise internal summary for review.",
        input:
          "Structured internal intake record",
        output:
          "Internal summary and classification",
        automation_policy:
          needsHumanApproval ||
          hasBlocker
            ? "assist_only"
      ...
      {
        id:
          "build_non_executing_preview",
        label:
          "Build non-executing preview",
        primitive: "reporting",
        description:
          "Generate a preview blueprint only. Do not execute external actions.",
        input:
          "Internal summary, risks, and missing details",
        output:
          "Non-executing automation blueprint preview",
        automation_policy:
          "blocked_in_mvp",
        risk_level: "low",
        approval_required: false,
        ...
        ? [
            {
              ...
              reason:
                "Sensitive or external actions must be reviewed before anything is sent, changed, refunded, deleted, or executed.",
              applies_to_step_ids: [
                "classify_and_summarize",
              ],
              required: true,
      ...
      input.risks.categories.map(
        (category) => ({
          ...
          risk_level:
            input.risks.risk_level,
          reason:
            "Detected by deterministic risk scanning.",
          recommendation:
            "Keep this workflow internal, draft-only, or human-reviewed depending on risk severity.",
        }),
      ),
    safe_to_automate: [
    ...
    safer_alternative:
      "Create an internal summary and route it to a human reviewer before any real-world action.",
  };
}
...
function normalizeAgentOutput(
  parsed: unknown,
  provider: AgentOutputProvider,
): NormalizedBlueprintArchitectOutput {
  const sanitized =
    sanitizeOptionalProposedRisks(
      parsed,
    );
  ...
  const output =
    blueprintArchitectOutputSchema.parse(
      {
        ...raw,
        provider,
        used_ai: true,
        ...
        reason:
          raw.reason ??
          "AI generated a structured blueprint proposal.",
      },
    );
...
function buildDebugInfo(
  ...
  return {
    ...
    used_ai:
      input.output.used_ai,
    fallback_used:
      input.output.fallback_used,
    status:
      input.output.status,
    llm_calls_made:
      input.llmCallsMade,
...
function logProviderFailure(
  ...
  console.error(
    `[blueprint architect:${provider}] Provider attempt failed`,
    {
      error:
...
export async function runBlueprintArchitectAgent(
  input:
    RunBlueprintArchitectAgentInput,
  ...
  if (!shouldUseAi(input.mode)) {
    const output =
      buildDeterministicBlueprintArchitectFallback(
        input,
        "deterministic",
        `Blueprint Architect Agent skipped in ${input.mode} mode.`,
      );
    ...
    try {
      ...
      rawResponse =
        await callGroqAgent(
          ...
          {
            modelEnv:
              "GROQ_BLUEPRINT_MODEL",
            fallbackModelEnv:
              "GROQ_AGENT_MODEL",
            maxTokensEnv:
              "GROQ_BLUEPRINT_MAX_TOKENS",
            fallbackMaxTokensEnv:
              "GROQ_AGENT_MAX_TOKENS",
            defaultMaxTokens: 2400,
            maxTokensCap: 4000,
            truncationSuggestion:
              "Raise GROQ_BLUEPRINT_MAX_TOKENS to around 2400-4000.",
          },
        );
        ...
        await callGeminiAgent(
          ...
          blueprintArchitectSystemPrompt,
          {
            ...
            fallbackModelEnv:
              "GEMINI_AGENT_MODEL",
            maxOutputTokensEnv:
              "GEMINI_BLUEPRINT_MAX_OUTPUT_TOKENS",
            fallbackMaxOutputTokensEnv:
            ...
            maxOutputTokensCap:
              4000,
            truncationSuggestion:
              "Raise GEMINI_BLUEPRINT_MAX_OUTPUT_TOKENS to around 2400-4000.",
          },
        );
  ...
  const providerErrors =
    providerAttempts
      ...
      .map(
        (attempt) =>
          `${attempt.provider}: ${attempt.error_summary}`,
      );
